...
void CReportCtrl::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult) 
{
    ...
    else if (lplvcd->nmcd.dwDrawStage == (CDDS_ITEMPREPAINT | CDDS_SUBITEM))
    {
		...
		if(!(lplvcd->iSubItem >= 0 && lplvcd->iSubItem < p->aTextColors.GetSize()))
			return;
...
BOOL CReportCtrl::_ItemCheckMonitor(int nIndex, BOOL bBefore, BOOL bAfter, UINT nMsg)
{
	...
	GetParent()->SendMessage(WM_ON_CHKBOX, (WPARAM)nIndex, (LPARAM)this);
	return TRUE;
}
...
void CReportCtrl::_MouseClkMonitor(UINT nMsg, UINT nFlags, CPoint point, BOOL bTriggerEdit)
{
	...
	case WM_LBUTTONDOWN:
		...
		GetParent()->SendMessage(WM_ON_CHKITEM, (WPARAM)IDX, (LPARAM)nMsg);
		break;

	case WM_LBUTTONDBLCLK:
		CListCtrl::OnLButtonDblClk(nFlags, point);
		GetParent()->SendMessage(WM_ON_DBCLKITEM, (WPARAM)IDX, (LPARAM)nCtrlIndex);
		break;
	...
	case WM_RBUTTONDOWN:
		...
		GetParent()->SendMessage(WM_ON_RCLKITEM, (WPARAM)IDX, (LPARAM)nCtrlIndex);
		break;
	...
	if (bTriggerEdit && m_bAllowEdit && !STATSCHANGED && !WAS_EDIT && WASACTIVE &&hti.iSubItem == 3)
...
void CReportCtrl::InvalidateCtrl()
{
	CRect rect;
